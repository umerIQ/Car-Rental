...
const Testimonial = () => {
  ...
  const addToTestimonialRef = (el) => {
    if (el && !testimonialRef.current.includes(el)) {
      testimonialRef.current.push(el);
    }
  };
  useEffect(() => {
    const container1 = container.current;
    const tittle = tittleRef.current;
    const testimonials = testimonialRef.current;
    gsap.set(tittle, {
      opacity: 0,
      y: 50,
    });
  ...
  return (
    <div ref={container} className="px-4 sm:px-20 xl:px-32 py-24">
      <div ref={tittleRef}>
        <Tittle
          tittle="What Our Customers Say"
          subTittle="Discover Why Discerning Travelers choose StayVanturen for their Luxury"
        />
      </div>
